...
// Lockear este mutex cuando se quiere usar el ultimo bloque
...
bool verificar_y_migrar_cadena(const Block *rBlock, const MPI_Status *status){
    ...
    printf("[%d] Pido cadena a %d, con indice %d y el hash %s\n", mpi_rank, status->MPI_SOURCE, rBlock->index, rBlock->block_hash);
    //MPI_Send(rBlock->block_hash, HASH_SIZE, MPI_CHAR, status->MPI_SOURCE, TAG_CHAIN_HASH, MPI_COMM_WORLD);
    MPI_Isend((void *)rBlock->block_hash, HASH_SIZE, MPI_CHAR, status->MPI_SOURCE, TAG_CHAIN_HASH, MPI_COMM_WORLD, &request);
    ...
    /** @corregir (ivan): Esto hay que chequear para todos, no sólo para el primero. */
    block_to_hash(&blockchain[0], hash_result);
    bool valid = true;
    if (hash_result != string(blockchain->block_hash)){
        ...
        for (int i = 1; i < bloques_recibidos; i++) {
            if (blockchain[i - 1].index != blockchain[i].index + 1){
                ...
                valid = false;
                break;
            } else if(string(blockchain[i - 1].previous_block_hash) != string(blockchain[i].block_hash)){
                printf("[%d]Indices %d no corresponde con indice anterior %d en la cadena recibida\n", mpi_rank, i, i-1);
                valid = false;
                break;
            }else{
                // (SANTI) aca estaria chequeando lo que no se chequeaba antes
                block_to_hash(&blockchain[i], hash_result);
                if (hash_result != blockchain[i].block_hash) {
                    printf("[%d]El del nodo %d no es igual al hash resultante de el mismo.\n", mpi_rank, i);
                    valid = false;
                    break;
                }
            }
            if (node_blocks.count(string(blockchain[i].block_hash)) == 1){
                // Acepto la cadena

                printf("[%d] ***ACEPTO*** la cadena de %d\n", mpi_rank, status->MPI_SOURCE);
    ...
    if (blockchain[bloques_recibidos-1].index == 1) {
        *last_block_in_chain = *rBlock;
        /** @corregir (ivan): Falta guardar los bloques nuevos en node_blocks. (Corregido)*/
        for (int j = 1; j < bloques_recibidos; j++) {
            node_blocks[string(blockchain[j].block_hash)] = blockchain[j];
        }
        delete []blockchain;
        return true;
...
bool validate_block_for_chain(const Block *rBlock, const MPI_Status *status){
    if(valid_new_block(rBlock)) {
        ...
        if (rBlock->index == last_block_in_chain->index + 1
            ...
            printf("[%d] Conflicto Fuerte: Perdí la carrera por (%d) contra %d \n", mpi_rank, rBlock->index, status->MPI_SOURCE);
...
void* proof_of_work(void *ptr){
    ...
    while(true){
        // Lockeo antes de obtener el bloque para asegurarme que no estoy recibiendo un nuevo bloque.
        pthread_mutex_lock(&usando_last_block);
        block = *last_block_in_chain;
        pthread_mutex_unlock(&usando_last_block);
    ...
    while(true){
        ...
        if(flag == 0){
            ...
            if(status.MPI_TAG == TAG_NEW_BLOCK) {
                ...
                MPI_Irecv(new_block, 1, *MPI_BLOCK, status.MPI_SOURCE, TAG_NEW_BLOCK, MPI_COMM_WORLD, &request);
                ...
                printf("[%d] Recibo pedido de envio de cadena para %d\n", mpi_rank, status.MPI_SOURCE);
                //MPI_Recv(block_hash, HASH_SIZE, MPI_CHAR, status.MPI_SOURCE, TAG_CHAIN_HASH, MPI_COMM_WORLD, &status);
                MPI_Irecv(block_hash, HASH_SIZE, MPI_CHAR, status.MPI_SOURCE, TAG_CHAIN_HASH, MPI_COMM_WORLD, &request);
                MPI_Wait(&request, &status);
        ...
        /** @corregir (ivan): Esta validación hay que hacerla en el thread de proof_of_work.
        * Sino, una vez que un nodo llega a MAX_BLOCKS deja de recibir pedidos de enviar bloques.
        * (tiene que seguir escuchando estos pedidos)
        * */
    }
